...
### Added
- **Full stdout/stderr of every user-facing script is now mirrored to `C:\zerotier_fix\run.log`** (or `%TEMP%\zerotier_fix_run.log` as fallback before the first install). Every BAT (`install_zerotier_gaming_fix.bat`, `uninstall_zerotier_gaming_fix.bat`, `Check_Network_interfaces.bat`, `resources/ZeroTier_Fix.bat`, `resources/change_MTU_only.bat`) self-re-executes through a PowerShell `Tee`-style pipe at the top — first run sets a marker var `ZGF_TEE_ACTIVE=1`, second run sees the marker and runs the real script while its stdout/stderr is mirrored to both the console and the log. Every run is bracketed with `===== <script> run start =====` / `===== run end =====` timestamps. This makes the full installer output, the scheduled-task SYSTEM-context output (which previously had no console at all), all diagnostic blocks and the summary table available for post-mortem debugging without having to reproduce the failure live.
- `install_zerotier_gaming_fix.bat` and `resources/change_MTU_only.bat`: `set /p` prompts split into a separate `echo`-line plus a prompt-less `set /p` because PowerShell pipes buffer the trailing-prompt form (no newline) until after Enter is pressed, hiding the prompt from the user. The split form prints the prompt as a full line — the pipe flushes it immediately — and `set /p` then reads from console stdin (which isn't piped) as normal.

### Changed
- `resources/ZeroTier_Fix.bat`: removed the legacy in-place `echo ... >> run.log` lines (`run start`, `run end`, `no ZT adapters found`) — they are now redundant with the tee-wrapper, which captures the same `echo` output and timestamps it from outside. The grep-able `ZeroTier_Fix.bat run start` / `run end` phrases are still emitted (now as plain `echo`, so they flow through the tee).

### Fixed
- **`resources/ZeroTier_Fix.bat`: ZT adapter detection silently failed under the tee wrapper, leaving every per-reconnect setting unapplied.** The original adapter-index lookup used `for /f "usebackq" %%I in (\`powershell ... Get-NetAdapter ^| Where-Object ...\`) ...` — the `^|` is the cmd-escape needed so the PS pipes don't terminate the backtick subcommand. With the new tee wrapper this script now runs inside an extra `cmd /c` layer, and the `^` escape was being consumed one layer too early. PowerShell received a literal `^|` and bailed with *"Es wurde kein Positionsparameter gefunden, der das Argument '^' akzeptiert."*, `ZT_IDX` came back empty, and the script took the `No ZeroTier adapters detected. Nothing to do.` early-exit — meaning Metric=1, NetworkCategory=Private, prefix policy, default-route deletion etc. **were never applied** when the task fired. This bug was technically present before the tee wrapper too — but invisible, because the failing PS output was discarded. Replaced with a temp-file round-trip (`powershell ... > %TEMP%\zgf_zt_idx.txt; set /p ZT_IDX=<...`) which keeps the pipes inside the quoted `-Command` string where cmd never tries to parse them.
- **ZeroTier-adapter `InterfaceMetric=1` was overwritten on the next reconnect.** `Set-NetIPInterface` was being called with `-InterfaceMetric 1` only; `AutomaticMetric` was left at its default `Enabled`, which makes Windows re-derive the metric from link speed (~35 for a 1 Gbps virtual adapter) on every network identification. Now also passes `-AutomaticMetric Disabled` and `-AddressFamily $_.AddressFamily`, so Metric=1 sticks.
- **ZeroTier `NetworkCategory=Private` flipped back to Public on the next reconnect.** `Set-NetConnectionProfile` writes the live category, but Windows' NLA service re-identifies ZT-style virtual adapters (no domain membership, no DC-issued DNS suffix) as Public on the next identification event. The fix now ALSO writes `Category=1` directly into the saved profile under `HKLM\…\NetworkList\Profiles\{guid}` matched by `ProfileName` — that is the on-disk value NLA reads back on reconnect, so the category survives.
- **`ZeroTier Auto Fix` scheduled task always reported `LastTaskResult=1`.** The script ended with bare `exit`, which propagates the last command's `errorlevel`. The legacy cleanup (`schtasks /delete /tn ZeroTier_PrioritizeIPv6 /f`) returns `1` on every fresh install because that legacy task doesn't exist, and that `1` was bleeding into the task result. Now ends with explicit `exit /b 0`.
- **`Check_Network_interfaces.bat` block `[3]` was always reported as FAIL by the summary even when the IPv6 prefix policy was correctly set.** `netsh interface ipv6 show prefixpolicies` prints the columns as **Precedence Label Prefix** (`100  4  ::ffff:0:0/96`), but the summary regex looked for `::ffff:0:0/96 100 4` — wrong order, never matched. Rewritten to find the line containing `::ffff:0:0/96` and verify that the *first* number on it is `100`.
- **`Check_Network_interfaces.bat` summary footer showed `WARN=` (empty) when there was exactly one warning.** PowerShell 5.1 returns a single `PSCustomObject` (not an array) from `Where-Object` when the filter matches one item, and `.Count` on that is unreliable. Counts are now `@(...).Count`, which always returns an int.
- **WinIPBroadcast install never worked since v2.4.0: the download URL 404'd.** The installer constructed `…/releases/download/winipbroadcast-1.6/WinIPBroadcast.exe`, but the upstream asset name is `WinIPBroadcast-1.6.exe` (version in filename). Local target filename stays `WinIPBroadcast.exe` because the service binary and the `WinIPBroadcast.exe install` subcommand expect that name.
- **WinIPBroadcast download had no retry and would give up on a single transient TLS hiccup** (`Die Anfrage wurde abgebrochen: Die Verbindung wurde unerwartet getrennt.`). Now retries up to 3 times with a 3 s backoff, enables both TLS 1.2 and TLS 1.3 (where the .NET enum exposes 1.3), and sends an explicit `User-Agent` since some CDN edges reject the PowerShell default.

---

## [v2.5.1] – 2026-05-14

Follow-up patch to v2.5.0. Fixes two real problems with the new diagnostic flow: a cmd parser error that aborted block `[8]` and (silently) prevented the summary table from running, plus the actual reason the window auto-closed on Explorer double-click. No installer or per-reconnect behavior change.

### Fixed
- **`Check_Network_interfaces.bat` block `[8]` aborted with a `"." kann syntaktisch an dieser Stelle nicht verarbeitet werden.` parser error, which also prevented the new summary table from running.** Unescaped parentheses in an `echo` line inside the `if errorlevel 1 ( … )` block (`echo … install_zerotier_gaming_fix.bat (Optional section).`) closed the `if`-block early for the cmd parser; the trailing `.` then ran as a bogus command. The parentheses are now escaped (`^(Optional section^)`).
